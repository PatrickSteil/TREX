...
inline std::vector<uint8_t> packBool(const std::vector<bool>& vector) noexcept {
    ...
    for (size_t i = 0; i < 8; i++) {
        resultValue =
            (resultValue << 1) | static_cast<uint8_t>((vectorIndex < vector.size()) ? (vector[vectorIndex]) : (false));
        vectorIndex++;
    }
    return result;
}
...
template <typename T>
std::vector<T> samplePercentage(const std::vector<T>& input, double percentage) {
    if (percentage <= 0.0) return {};       // empty vector for 0%
    if (percentage >= 100.0) return input;  // return all elements for >=100%
