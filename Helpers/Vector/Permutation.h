...
protected:
    ...
    IdMapping(const Construct::IdTag, const size_t n) : std::vector<size_t>() {
        reserve(n);
    ...
    IdMapping(const Construct::RandomTag, const size_t n, const int seed = 42) : IdMapping(Construct::Id, n) {
    ...
    template <typename T>
    IdMapping(const Construct::SortTag, const std::vector<T>& vector) : IdMapping(Construct::Id, vector.size()) {
    ...
    template <typename T>
    IdMapping(const Construct::InvertTag, const T& original) : std::vector<size_t>(original.size()) {
    ...
    template <typename T>
    IdMapping(const Construct::InvertTag, T&& original) : std::vector<size_t>(std::move(original)) {
...
public:
    ...
    inline void serialize(IO::Serialization& serialize) const noexcept { serialize(vector()); }

    inline void deserialize(IO::Deserialization& deserialize) noexcept { deserialize(vector()); }

    inline void serialize(const std::string& fileName) const noexcept { IO::serialize(fileName, vector()); }

    inline void deserialize(const std::string& fileName) noexcept { IO::deserialize(fileName, vector()); }
};
...
public:
    ...
    Permutation(const Construct::IdTag, const size_t n) : IdMapping(Construct::Id, n) {}

    Permutation(const Construct::RandomTag, const size_t n, const int seed = 42)
        : IdMapping(Construct::Random, n, seed) {}

    Permutation(const Construct::InvertTag, const Order& order) : IdMapping(Construct::Invert, order) {}

    Permutation(const Construct::InvertTag, Order&& order) : IdMapping(Construct::Invert, std::move(order)) {}
...
public:
    ...
    inline std::vector<T> getPermuted(const std::vector<T>& vector) const noexcept {
        AssertMsg(vector.size() == size(), "Cannot permute a vector of size "
                                               << vector.size() << " with a permutation of size " << size() << "!");
...
public:
    ...
    Order(const Construct::IdTag, const size_t n) : IdMapping(Construct::Id, n) {}

    Order(const Construct::RandomTag, const size_t n, const int seed = 42) : IdMapping(Construct::Random, n, seed) {}

    template <typename T>
    Order(const Construct::SortTag, const std::vector<T>& vector) : IdMapping(Construct::Sort, vector) {}

    template <typename T, typename COMPARE>
    Order(const Construct::SortTag, const std::vector<T>& vector, const COMPARE& comp)
        : IdMapping(Construct::Sort, vector, comp) {}

    Order(const Construct::InvertTag, const Permutation& permutation) : IdMapping(Construct::Invert, permutation) {}
