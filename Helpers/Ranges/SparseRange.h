...
public:
    ...
    SparseRange() : flag(nullptr), beginIndex(0), endIndex(0) {}

    SparseRange(const std::vector<bool>& flag) : flag(&flag), beginIndex(0), endIndex(flag.size()) {
    ...
    inline Iterator end() const noexcept { return Iterator(flag, (flag) ? Element(flag->size()) : beginIndex); }

    inline bool empty() const noexcept { return endIndex <= beginIndex; }

    inline size_t size() const noexcept { return (flag) ? Vector::count(*flag, true) : 0; }
