...
public:
    ...
    public:
        ...
        inline Iterator& validate() noexcept {
            ...
            while ((edgeA != outer->rangeA.end()) &&
                   (outer->graphA->get(ToVertex, *edgeA) != outer->graphB->get(ToVertex, *edgeB))) {
    ...
    EdgeIntersectionRange() : graphA(nullptr), graphB(nullptr) {}

    EdgeIntersectionRange(const GraphA* const graphA, const GraphB* const graphB, const Vertex vertex)
        : graphA(graphA), graphB(graphB), rangeA(graphA->edgesFrom(vertex)), rangeB(graphB->edgesFrom(vertex)) {}

    inline Iterator begin() const noexcept { return Iterator(this, rangeA.begin(), rangeB.begin()).validate(); }

    inline Iterator end() const noexcept { return Iterator(this, rangeA.end(), rangeB.end()); }
