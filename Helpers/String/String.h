...
inline bool isWhiteSpace(const char c) { return (c == ' ') || (c == '\t') || (c == '\n'); }
...
template <typename T>
inline std::string nsToString(T t, T infinity = std::numeric_limits<T>::max()) {
    if (t >= infinity) return "infinity";
    if ((!(t >= 0)) && (t <= -infinity)) return "-infinity";
    if (t == 0) return "0ns";

    std::string result = std::to_string(t % 1000) + "ns";
    if (t >= 1000) {
        std::string fillIn = "";
        if ((t % 1000) < 10)
            fillIn = "  ";
            ...
            fillIn = " ";
        result = musToString(t / 1000) + " " + fillIn + result;
    }
    return result;
}

inline std::string nsToString(double t) { return nsToString<int>(t); }
inline std::string nsToString(double t, int infinity) { return nsToString<int>(t, infinity); }
...
inline int parseSeconds(const std::string& time) {
    AssertMsg(time.length() == 8 || time.length() == 9, "Time " << time << " has not the correct length!");
    int seconds = 0;
    int value = 0;
