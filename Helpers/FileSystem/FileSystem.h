...
inline bool isAbsolutePath(const std::string& path) noexcept { return (!path.empty()) && (path[0] == '/'); }
...
inline bool isFileOrDirectory(const std::string& path) noexcept { return isFile(path) || isDirectory(path); }
...
inline void unzip(const std::string& zipFileName, const std::string& ouputDirectory, const bool verbose = true,
    ...
    switch (pid) {
        ...
        case 0:
            if (verbose) {
                execl(unzipExecutable.c_str(), "unzip", "-o", zipFileName.c_str(), "-d", ouputDirectory.c_str(),
                      nullptr);
            } else {
