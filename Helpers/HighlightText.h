...
public:
    ...
    StreamWrapper(const StreamWrapper& stream) : flushed(false) { stream.flush(text); }
...
inline std::ostream& operator<<(std::ostream& out, const StreamWrapper& stream) { return out << (std::string)stream; }
