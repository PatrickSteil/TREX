...
public:
    static bool isComment(const char* line) { return isCommentStartChar(*line, COMMENT_START_CHAR_LIST...); }
};
...
public:
    ...
    explicit CSVReader(const std::string& fileName) : in(fileName) { init(); }

    explicit CSVReader(const std::vector<std::string>& fileNameAliases) : in(fileNameAliases) { init(); }

    template <typename... T, typename = std::enable_if_t<sizeof...(T) == COLUMN_COUNT>>
    void readHeader(const T&... columnNames) {
        columnNameAliases =
            std::array<std::vector<std::string>, COLUMN_COUNT>{std::vector<std::string>{columnNames}...};
