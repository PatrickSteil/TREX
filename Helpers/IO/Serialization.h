...
public:
    ...
    Serialization(const std::string& fileName, const Ts&... objects)
        : fileName(FileSystem::ensureDirectoryExists(fileName)), os(fileName, std::ios::binary) {
...
private:
    ...
    inline void serialize(const std::vector<T>& vectorObject) noexcept {
        ...
        } else if constexpr (IsSerializable<T>() || IsVectorType<T>() || IsArrayType<T>() ||
                             Meta::Equals<T, std::string>()) {
...
public:
    ...
    inline void version(const size_t expectedVersionId) noexcept {
        ...
        Ensure(fileVersionId == expectedVersionId, "Expected version " << expectedVersionId << ", but file " << fileName
...
private:
    ...
    inline void deserialize(std::vector<T>& vectorObject) noexcept {
        ...
        std::string type;
        deserialize(type);
        Ensure(type == Meta::type<T>(), "Trying to deserialize an std::vector<"
                                            << Meta::type<T>() << "> from a file that contains an std::vector<" << type
                                            << ">!");
    ...
    inline void deserialize(std::array<T, N>& arrayObject) noexcept {
        ...
        std::string type;
        deserialize(type);
        Ensure(type == Meta::type<T>(), "Trying to deserialize an std::array<"
                                            << Meta::type<T>() << "> from a file that contains an std::array<" << type
                                            << ">!");
        decltype(arrayObject.size()) size = 0;
        deserialize(size);
        Ensure(size == N, "Trying to deserialize an array of size "
                              << N << " from a file that contains an array of size " << size << "!");
        if constexpr (IsSerializable<T>() || IsVectorType<T>() || IsArrayType<T>() || Meta::Equals<T, std::string>()) {
