...
public:
    Cd(BasicShell& shell) : ParameterizedCommand(shell, "cd", "Changes the current working directory of the shell.") {
...
private:
    inline void error(const std::string& s) { shell << "Unknown path / wrong syntax (" << s << ")." << newLine; }
};
