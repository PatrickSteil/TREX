...
class ParameterizedCommand : public Command {
 ...
 public:
  ...
  virtual void execute(const std::string& parameter) noexcept {
    std::vector<std::string> tokens =
        String::split(String::trim(parameter), ' ');
    ...
    for (size_t i = 0; i < parameters.size(); i++) {
      if (i < tokens.size()) {
        ...
        if (parameters[i].options.empty()) {
          parameters[i].value =
              shell.ask(parameters[i].name + "> ", parameterSuggestions(i));
        } else {
          parameters[i].value =
              shell.ask(parameters[i].name + "> ", parameters[i].options);
        }
      ...
      if ((!parameters[i].options.empty()) &&
          (!Vector::contains(parameters[i].options, parameters[i].value))) {
        shell.error("Parameter <", parameters[i].name,
 ...
 protected:
  ...
  inline void addParameterToDescription(const Parameter& p) {
    if (!p.options.empty()) {
      description = description + DescriptionLineBreak + "    <" + p.name +
                    ">: " + p.optionsString();
    }
  }
