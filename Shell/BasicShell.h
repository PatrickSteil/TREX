...
public:
    ...
    inline std::string ask(const std::string& question) { return ask<std::string>(question); }
...
public:
    ...
    inline void clearLine() const { shell << "\r" << whiteSpace(getScreenWidth()) << "\r"; }
    ...
    inline void setCacheFile(const std::string& filename) { cacheFile = filename; }
