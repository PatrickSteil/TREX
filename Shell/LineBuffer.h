...
public:
    LineBuffer(std::ostream& out) : out(out), firstCharColumn(0), cursorCouldBeOffScreen(false) {}
    ...
    inline std::string getString() const noexcept { return getPrefix() + getSuffix(); }
...
private:
    ...
    inline void moveCursorLeft(const size_t textSize, const size_t amount = 1) noexcept {
        if (amount == 0) return;
        AssertMsg(textSize >= amount, "Cannot move left for " << amount << " steps, because the buffer contains only "
                                                              << textSize << " chars!");
