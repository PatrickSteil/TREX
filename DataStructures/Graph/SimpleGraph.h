...
template <typename VertexType = std::size_t>
struct SimpleGraph {
    ...
    SimpleGraph(const SimpleGraph& other) : adjArray(other.adjArray), toVertex(other.toVertex){};

    SimpleGraph(SimpleGraph&& other) noexcept
        : adjArray(std::move(other.adjArray)), toVertex(std::move(other.toVertex)) {}
    ...
    void fromEdgeList(std::vector<std::tuple<VertexType, VertexType, uint16_t>>& edges, std::size_t numVertices) {
        clear();
    ...
    std::size_t memoryConsumption() const {
        return adjArray.capacity() * sizeof(std::size_t) + toVertex.capacity() * sizeof(VertexType);
    }
};
