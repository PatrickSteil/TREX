...
public:
    explicit TimestampedIndexedMap(std::size_t capacity) : index(capacity), stamp(capacity, 0), currentStamp(1) {}
    ...
    inline Value getOrDefault(KeyType key) const noexcept {
        if ((std::size_t)key >= index.size()) return DefaultValue;
        if (stamp[key] != currentStamp) return DefaultValue;
        return values[index[key]];
    }
